...
import { Play, Pause, Disc3, Clock, MoreHorizontal, Edit, Trash2 } from 'lucide-react';
        ...
        <div className="relative">
            ...
            <div className="h-80 w-full relative flex items-end p-8 bg-gradient-to-b from-zinc-800 to-zinc-950/90">
                <div className="relative z-20 flex items-end gap-6 w-full">
                    {album.imageUrl ? (
                        <img
                            src={album.imageUrl.startsWith('http') ? album.imageUrl : `http://localhost:5000${album.imageUrl}`}
                            alt={album.title}
            ...
            <div className="p-8 pb-6 flex items-center gap-6 bg-gradient-to-b from-zinc-950/90 to-transparent">
                ...
                {isAdmin && (
                    <div className="flex gap-4 items-center">
                        <Button variant="outline" size="sm" onClick={() => navigate(`/albums/edit/${album._id}`)}>
                ...
                {songs.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-1">
                        {songs.map((song, index) => (
                            <div
                                key={song._id}
                                className="group grid grid-cols-[16px_1fr_120px_minmax(120px,200px)] gap-4 px-4 py-3 rounded-md hover:bg-zinc-800/50 transition-colors items-center cursor-pointer"
                            >
                                <div className="text-center text-zinc-400 text-sm">{index + 1}</div>
                                <div>
